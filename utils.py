...
# --- FUNCTION UPDATED ---
# This function is updated to accept 'channel_id'
# and checks the user's status in that specific channel.
async def is_req_subscribed(client, message, channel_id):
    if not channel_id:
        return True
    try:
        user = await client.get_chat_member(int(channel_id), message.from_user.id)
        if user.status in [enums.ChatMemberStatus.MEMBER, enums.ChatMemberStatus.OWNER, enums.ChatMemberStatus.ADMINISTRATOR]:
            return True
        else:
            return False
        ...
        logger.error(f"Error checking subscription for {message.from_user.id} in {channel_id}: {e}")
        return False
# --- END OF UPDATE ---
