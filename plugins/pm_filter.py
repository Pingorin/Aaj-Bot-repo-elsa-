...
async def pm_search(client, message):
    if IS_PM_SEARCH:
        ...
        await message.reply_text("<b>⚠️ ꜱᴏʀʀʏ ɪ ᴄᴀɴ'ᴛ ᴡᴏʀᴋ ɪN ᴘᴍ</b>")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # FIX: Purana referral block poori tarah delete kar diya gaya hai
    ...
    # --- FIX: 'checksub' LOGIC KO NAYE SEQUENTIAL FLOW KE LIYE UPDATE KIYA GAYA ---
    elif query.data.startswith("checksub"):
        ident, grp_id, file_id = query.data.split("#")
        
        # Naya FSub Logic Yahan Lagaya (Sequential Check)
        fsub_pass, failed_channel = await check_fsub(client, query) # 'query' (CallbackQuery) 'from_user' attribute rakhta hai
        ...
        if not fsub_pass:
            ...
            try:
                ...
                logger.error(f"Error creating invite link in checksub: {e}")
                await query.answer("Ek error aa gaya, admin se contact karein.", show_alert=True)
                return
            ...
            await query.answer("Aapne abhi bhi channel join nahi kiya hai!", show_alert=True)
            
            # Message ko naye button ke saath update karein
            try:
                await query.message.edit_text(
                    text="🙁 ғɪʀꜱᴛ ᴊᴏɪɴ ᴏᴜʀ ᴄʜᴀɴɴᴇʟ ᴛʜᴇɴ ʏᴏᴜ ᴡɪʟʟ ɢᴇᴛ ᴍᴏᴠɪᴇ.\n\nᴄʟɪᴄᴋ ᴊᴏɪɴ ɴᴏᴡ ʙᴜᴛᴛᴏN, ꜱᴇɴᴅ ʀᴇǫᴜᴇꜱᴛ, ᴀɴᴅ ᴛʜᴇɴ ᴄʟɪᴄᴋ 'ᴛʀʏ ᴀɢᴀɪɴ'.",
                    reply_markup=InlineKeyboardMarkup(btn),
