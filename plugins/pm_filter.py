...
import pyrogram
# FIX: Naye FSub ke liye AUTH_CHANNEL_2 add kiya
from info import (
    MAX_BTN, BIN_CHANNEL, USERNAME, URL, ADMINS, LANGUAGES, AUTH_CHANNEL, AUTH_CHANNEL_2, 
...
# FIX: 'is_req_subscribed' ko 'check_fsub' se replace kiya
from utils import (
    temp, get_settings, is_check_admin, get_status, get_hash, get_name, 
    get_size, save_group_settings, check_fsub, get_poster, get_readable_time
)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # (Poora 'elif query.data.startswith("referral")' block yahan se delete kar diya gaya hai)
    ...
    # --- FIX: YAHAN SE NAYA 'checksub' LOGIC SHURU HOTA HAI ---
    elif query.data.startswith("checksub"):
        ident, grp_id, file_id = query.data.split("#")
        
        # Naya FSub Logic Yahan Lagaya
        fsub_pass, failed_channels_list = await check_fsub(client, query) # 'query' (CallbackQuery) 'from_user' attribute rakhta hai
        
        if not fsub_pass:
            # User abhi bhi check fail kar raha hai
            btn = [] # Naya button list
            
            for channel_id in failed_channels_list:
                try:
                    invite_link = await client.create_chat_invite_link(int(channel_id), creates_join_request=True)
                    channel_name = "Channel 1"
                    if channel_id == AUTH_CHANNEL_2:
                        channel_name = "Channel 2"
                    
                    btn.append(
                        [InlineKeyboardButton(f"⛔️ ᴊᴏɪɴ {channel_name} ⛔️", url=invite_link.invite_link)]
                    )
                except Exception as e:
                    logger.error(f"Error creating invite link for {channel_id} in checksub: {e}")
            
            # 'Try Again' button ko wapas add karein
            btn.append(
                [InlineKeyboardButton("♻️ ᴛʀʏ ᴀɢᴀɪɴ ♻️", callback_data=f"checksub#{grp_id}#{file_id}")]
            )
            
            # User ko alert karein
            await query.answer("Aapne abhi bhi sabhi channels join nahi kiye hain!", show_alert=True)
            
            # Message ko naye buttons ke saath update karein
            try:
                await query.message.edit_text(
                    text="🙁 ᴀᴀᴘᴋᴏ ꜰɪʟᴇ ʟᴇɴᴇ ᴋᴇ ʟɪʏᴇ ɴɪᴄʜᴇ ᴅɪʏᴇ ɢᴀʏᴇ **ꜱᴀʙʜɪ** ᴄʜᴀɴɴᴇʟꜱ ᴋᴏ ᴊᴏɪɴ ᴋᴀʀɴᴀ ᴢᴀʀᴜʀɪ ʜᴀɪ.\n\nᴊᴏɪɴ ᴋᴀʀᴋᴇ 'ᴛʀʏ ᴀɢᴀɪɴ' ᴘᴀʀ ᴄʟɪᴄᴋ ᴋᴀʀᴇɪN.",
                    reply_markup=InlineKeyboardMarkup(btn),
                    parse_mode=enums.ParseMode.HTML
                )
            except MessageNotModified:
                pass # Agar user spam click kare toh error na aaye
            return # Yahin ruk jaayein
        
        # --- FSub Logic End ---

        # Agar user yahan tak pohochta hai, matlab usne check pass kar liya hai
        await query.answer("✅ Check successful! Sending file...", show_alert=False)
        ...
        # "Join Channel" message ko delete karein
        try:
            await query.message.delete()
        except:
            pass
    ...
    # --- FIX: NAYA 'checksub' LOGIC YAHAN KHATAM HOTA HAI ---
