...
# --- FSUB UPDATE: IMPORTS ---
from info import (
    MAX_BTN, BIN_CHANNEL, USERNAME, URL, ADMINS, LANGUAGES, 
    # AUTH_CHANNEL, <-- Purana Fsub, Hata diya
    SUPPORT_GROUP, IMDB, IMDB_TEMPLATE, LOG_CHANNEL, LOG_VR_CHANNEL, TUTORIAL, FILE_CAPTION, 
    SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, SHORTENER_API2, IS_PM_SEARCH, 
    QR_CODE, DELETE_TIME, REFERRAL_TARGET,
    # Naye Fsub variables
    AUTH_CHANNEL_1, AUTH_CHANNEL_2
)
# Naya Fsub helper function
try:
    from plugins.fsub_handler import build_fsub_check
except ImportError:
    logger.error("Fsub handler (plugins/fsub_handler.py) nahi mila. Fsub logic kaam nahi karega.")
    build_fsub_check = None # Safety
# --- END FSUB UPDATE ---
...
# ... (pm_search, group_search, next_page, language handlers, etc. - koi badlaav nahi) ...
# ... (Yahan aapke saare functions hain jaise pm_search, group_search, next_page, ...)
# ... (languages_cb_handler, lang_search, quality_filter_cb_handler, set_quality_cb_handler, ...)
# ... (years_cb_handler, set_year_cb_handler, advantage_spoll_choker, ...)
# ... (Hum seedha cb_handler par ja rahe hain jahan badlaav hai)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # --- FSUB UPDATE: Poora `checksub` handler naye logic se replace karein ---
    elif query.data.startswith("checksub"):
        ident, grp_id, file_id = query.data.split("#")
        
        if build_fsub_check and AUTH_CHANNEL_1 and AUTH_CHANNEL_2:
            # Naya Fsub logic
            fsub_status, fsub_message, fsub_buttons = await build_fsub_check(
                client, query.from_user.id, file_id, int(grp_id) # grp_id ko naye "Try Again" button mein pass karein
            )
            ...
            if fsub_status == "FAIL":
                ...
                await query.answer("File access denied. Please join the channels.", show_alert=True)
                return
            
            # Agar PASS hai, toh file bhejenge
            final_caption_message = f"**{fsub_message}**\n\n" if fsub_status == "PENDING" else ""
        else:
            # Fallback agar Fsub disabled hai
            final_caption_message = ""
            # Purana Fsub check (is_req_subscribed) yahan se hata diya gaya hai
            # Kyunki naya logic use replace kar raha hai
            pass
            
        # File bhejme ka logic (pehle se hi tha)
        ...
        # Purane "Try Again" message ko delete kar dein
        await query.message.delete()

    # --- FSUB UPDATE: Naya handler add karein ---
    elif query.data.startswith("check_fsub#"):
        # Yeh naye "Try Again" button (check_fsub#{grp_id}#{file_id}) ko handle karega
        try:
            ident, grp_id, file_id = query.data.split("#")
            grp_id = int(grp_id)
        except:
            # Fallback agar grp_id nahi mila (purane logic se)
            ident, file_id = query.data.split("#")
            grp_id = temp.CHAT.get(query.from_user.id, 0) # temp.CHAT se try karein
            if grp_id == 0:
                await query.answer("Error: Group ID not found. Please try the request again from the group.", show_alert=True)
                return
        ...
        if build_fsub_check and AUTH_CHANNEL_1 and AUTH_CHANNEL_2:
            ...
            await query.answer("Fsub system is disabled or configured incorrectly.", show_alert=True)
            return
        ...
        files = files_[0]
        settings = await get_settings(grp_id) # Sahi grp_id use karein
        ...
        await client.send_cached_media(
            ...
            file_id=file_id,
            caption=final_caption,
            protect_content=settings['file_secure'], # Sahi settings use karein
            reply_markup=InlineKeyboardMarkup(btn)
        )
...
async def auto_filter(client, msg, spoll=False):
    ...
    # --- FSUB LOGIC FOR AUTO_FILTER ---
    # `auto_filter` sirf results dikhata hai, file send nahi karta.
    # File send karne ka logic `commands.py` (start=file_...) handle karta hai.
    # Isliye Fsub check `commands.py` mein hai, yahan nahi.
    # Yahan logic add karne se user ko file link milne se pehle hi Fsub karna padega,
    # jo sahi hai, par aapka current setup `commands.py` par depend karta hai.
    # Hum ise `commands.py` mein hi rakhenge.
    # --- END FSUB LOGIC ---
