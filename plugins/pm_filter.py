...
import pyrogram
# FIX: Added REFERRAL_TARGET, AUTH_CHANNEL, REQ_AUTH_CHANNEL
from info import (
    MAX_BTN, BIN_CHANNEL, USERNAME, URL, ADMINS, LANGUAGES, AUTH_CHANNEL, REQ_AUTH_CHANNEL, SUPPORT_GROUP, IMDB, 
...
# NAYA F-SUB CHECK PM MEIN
@Client.on_message(filters.private & filters.text & filters.incoming)
async def pm_search(client, message):
    # --- YAHAN SE F-SUB CHECK SHURU HOTA HAI ---
    ...
    if not auth_subscribed or not req_auth_subscribed:
        btn = []
        text = "🙁 <b>Search karne se pehle, kripya neeche diye gaye channel(s) ko join karein...</b>\n\n"
        
        if not auth_subscribed:
            try:
                invite_link = await client.create_chat_invite_link(int(AUTH_CHANNEL), creates_join_request=True)
                btn.append([InlineKeyboardButton("⛔️ ᴊᴏɪɴ ɴᴏᴡ ⛔️", url=invite_link.invite_link)])
                text += "<b>1. Hamaara main channel join karein.</b>\n"
            except ChatAdminRequired:
                logger.error(f"Bot ko AUTH_CHANNEL mein admin banayein: {AUTH_CHANNEL}")

        if not req_auth_subscribed:
            try:
                invite_link = await client.create_chat_invite_link(int(REQ_AUTH_CHANNEL), creates_join_request=True)
                btn.append([InlineKeyboardButton("✨ ᴅᴀɪʟʏ ᴜᴘᴅᴀᴛᴇ ✨", url=invite_link.invite_link)])
                text += "<b>2. Hamaara 'Daily Update' channel join karein.</b>\n"
            except ChatAdminRequired:
                logger.error(f"Bot ko REQ_AUTH_CHANNEL mein admin banayein: {REQ_AUTH_CHANNEL}")

        text += "\n...aur fir 'Try Again' button par click karein."
        
        btn.append(
            [InlineKeyboardButton("♻️ ᴛʀʏ ᴀɢᴀɪɴ ♻️", callback_data="checksub_pm")]
        )
        ...
        return
    # --- F-SUB CHECK YAHAN KHATAM HOTA HAI ---
...
async def group_search(client, message):
    ...
    if settings["auto_filter"]:
        ...
        if not auth_subscribed or not req_auth_subscribed:
            ...
            btn.append(
                [InlineKeyboardButton("♻️ ᴛʀʏ ᴀɢᴀɪɴ ♻️", callback_data="checksub_grp")]
            )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # NAYE "TRY AGAIN" BUTTON HANDLERS
    elif query.data == "checksub_grp":
        # Group waale "Try Again" ke liye
        auth_subscribed = not AUTH_CHANNEL or await db.find_join_req(query.from_user.id, AUTH_CHANNEL)
        req_auth_subscribed = not REQ_AUTH_CHANNEL or await db.find_join_req(query.from_user.id, REQ_AUTH_CHANNEL)

        if not auth_subscribed:
            return await query.answer("Kripya pehle 'Join Now' (Main) channel par request bhejein...", show_alert=True)
        if not req_auth_subscribed:
            return await query.answer("Kripya 'Daily Update' channel par bhi request bhejein...", show_alert=True)
        
        # Agar dono join kar liye hain, to F-sub message delete karein
        await query.message.delete()
        await query.answer("Shukriya! Ab aap group mein fir se search kar sakte hain.", show_alert=True)

    elif query.data == "checksub_pm":
        # PM waale "Try Again" ke liye
        auth_subscribed = not AUTH_CHANNEL or await db.find_join_req(query.from_user.id, AUTH_CHANNEL)
        req_auth_subscribed = not REQ_AUTH_CHANNEL or await db.find_join_req(query.from_user.id, REQ_AUTH_CHANNEL)
        ...
        # Agar dono join kar liye hain, to F-sub message delete karein
        await query.message.delete()
        await query.answer("Shukriya! Ab aap apni movie ka naam fir se search kar sakte hain.", show_alert=True)
    
    # FILE LINK WALA "TRY AGAIN"
