...
import pyrogram
from info import (
    MAX_BTN, BIN_CHANNEL, USERNAME, URL, ADMINS, LANGUAGES, AUTH_CHANNEL, 
...
async def next_page(bot, query):
    ...
    # Add Referral Button
    btn.append(
        [InlineKeyboardButton("💰 ʀᴇꜰᴇʀʀᴀʟ", callback_data="referral")]
    )
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    buttons.append([InlineKeyboardButton(text="⪻ ʙᴀᴄᴋ ᴛᴏ ᴍᴀɪN ᴘᴀɢᴇ", callback_data=f"next_{req}_{key}_{offset}")])
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "referral":
        try:
            buttons = [[
                InlineKeyboardButton("🔗 sʜᴀʀᴇ ʟɪɴᴋ", switch_inline_query=f"https://t.me/{temp.U_NAME}?start=ref_{query.from_user.id}"),
                InlineKeyboardButton("🎁 ᴄʟᴀɪᴍ ʀᴇᴡᴀʀᴅ", callback_data="ref_claim")
            ],[
            ...
            referral_count = user_info.get('referrals', 0) if user_info else 0
            claimed_count = user_info.get('referral_target_count', 0) if user_info else 0
            
            # Calculate current referrals since last claim
            current_referrals = referral_count - (claimed_count * REFERRAL_TARGET)
            
            await query.message.edit_text(
                script.REFERRAL_TXT.format(
    ...
    elif query.data == "ref_claim":
        try:
            user_info = await db.get_user_info(query.from_user.id)
            if not user_info:
                await query.answer("Could not find your user data. Please /start the bot again.", show_alert=True)
                return

            referral_count = user_info.get('referrals', 0)
            claimed_count = user_info.get('referral_target_count', 0)
            
            # Calculate how many referrals are needed for the *next* reward
            needed_referrals = (claimed_count + 1) * REFERRAL_TARGET
            
            if referral_count >= needed_referrals:
                # User is eligible for a new reward
                
                # Grant 1 month premium
                expiry_time = datetime.now() + timedelta(days=30)
                # Use update_user for premium (this function is in your db, it updates 'uersz' collection)
                await db.update_user({"id": query.from_user.id, "expiry_time": expiry_time})
                
                # Update their claimed count in the main user collection ('col')
                await db.update_referral_claim(query.from_user.id, claimed_count + 1)
                
                await query.answer(script.REFERRAL_CLAIM_SUCCESS_TXT, show_alert=True)
                
                # Update the referral message to show new count
                current_referrals = referral_count - ( (claimed_count + 1) * REFERRAL_TARGET )
                buttons = [[
                    InlineKeyboardButton("🔗 sʜᴀʀᴇ ʟɪɴᴋ", switch_inline_query=f"https://t.me/{temp.U_NAME}?start=ref_{query.from_user.id}"),
                    InlineKeyboardButton("🎁 ᴄʟᴀɪᴍ ʀᴇᴡᴀʀᴅ", callback_data="ref_claim")
                ],[
                ...
                # Not enough referrals for the next target
                current_referrals = referral_count - (claimed_count * REFERRAL_TARGET)
                await query.answer(
                    script.REFERRAL_NOT_ENOUGH_TXT.format(
                        current_referrals=current_referrals,
                        referral_target=REFERRAL_TARGET
                    ), 
                    show_alert=True
            ...
            print(f"Error in ref_claim callback: {e}")
            await query.answer("An error occurred while claiming.", show_alert=True)
