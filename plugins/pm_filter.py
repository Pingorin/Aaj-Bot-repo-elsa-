...
import pyrogram
# FIX: Added REFERRAL_TARGET and removed AUTH_CHANNEL
from info import (
    ...
    REFERRAL_TARGET, REQ_SUB_CHANNEL_1, REQ_SUB_CHANNEL_2
)
...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    # FIX: Added 'How to Download' button
    btn.append(
        [InlineKeyboardButton("🤔 ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ 🤔", url=settings['tutorial'])]
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # --- FIX: YEH POORA BLOCK AB OBSOLETE/BEKAAR HAI ---
    # Humne FSub logic ko 'commands.py' mein move kar diya hai
    # aur 'Try Again' button ab 'url' ka istemal karta hai, 'callback_data' ka nahi.
    # Isliye 'checksub' ko delete kar diya gaya hai.
...
async def auto_filter(client, msg, spoll=False):
    ...
    if len(available_years) > 1:
        filter_buttons.append(
            InlineKeyboardButton("📅 YᴇᴀR", callback_data=f"years#{key}#0#{req}")
        )
