...
import pyrogram
# FIX: Added REFERRAL_TARGET and new REQ_SUB channels
from info import (
    MAX_BTN, BIN_CHANNEL, USERNAME, URL, ADMINS, LANGUAGES, SUPPORT_GROUP, IMDB, 
    ...
    REFERRAL_TARGET, REQ_SUB_CHANNEL_1, REQ_SUB_CHANNEL_2  # <-- FIX 1: AUTH_CHANNEL hataya, naye channels daale
)
...
# FIX 2: 'is_req_subscribed' ko yahan se hata diya gaya hai
from utils import temp, get_settings, is_check_admin, get_status, get_hash, get_name, get_size, save_group_settings, get_poster, get_status, get_readable_time
...
async def next_page(bot, query):
    ...
    btn.insert(0,[
        ...
        InlineKeyboardButton("🥇ʙᴜʏ🥇", url=f"https.me/{temp.U_NAME}?start=buy_premium"),
    ...
    btn.append(
        [InlineKeyboardButton("💰 ʀᴇꜰᴇʀ & ᴇᴀʀɴ 💰", url=f"https.me/{temp.U_NAME}?start=get_referral_{query.message.chat.id}")]
    )
    ...
    if settings["link"]:
        links = ""
        for file_num, file in enumerate(files, start=offset+1):
            links += f"""<b>\n\n{file_num}. <a href=https.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{file.file_id}>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}</a></b>"""
...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    # --- FIX: "How to Download" button pichle step mein add kiya gaya tha ---
    btn.append(
        [InlineKeyboardButton("🤔 ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ 🤔", url=settings['tutorial'])]
    )
    # --- FIX ENDS ---
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # --- FIX 3: YEH POORA 'checksub' BLOCK REPLACE KIYA GAYA HAI ---
    elif query.data.startswith("checksub"):
        ident, grp_id, file_id = query.data.split("#")
        settings = await get_settings(int(grp_id))

        # --- NAYA JOIN REQUEST FSUB CHECK ---
        # Yeh check karta hai ki user ne "Try Again" button dabane se pehle request bheji ya nahi
        ...
        # Agar abhi bhi request nahi bheji hai, toh usey dobara links dikhayein
        if not has_req_1 or not has_req_2:
            ...
            try:
                ...
                logger.error(f"checksub mein invite link banate waqt error: {e}")
                await query.answer("Bot configuration error. Admin se sampark karein.", show_alert=True)
                return
            ...
            # Wahi "Try Again" button
            btn.append(
                [InlineKeyboardButton("♻️ Request Bhejne Ke Baad Yahan Click Karein ♻️", callback_data=f"checksub#{grp_id}#{file_id}")]
            )

            # User ko alert karein
            try:
                await query.message.edit_text(
                    (
                        "**❌ Abhi Bhi Pending Hai ❌**\n\n"
                        "Aapne abhi tak sabhi channels par join request nahi bheji hai.\n\n"
                        "Kripya pehle request bhejein, fir 'Request Bhejne Ke Baad Yahan Click Karein' button dabayein."
                    ),
                    reply_markup=InlineKeyboardMarkup(btn),
                    parse_mode=enums.ParseMode.MARKDOWN
                )
            except MessageNotModified:
                pass # Agar message same hai toh error na dikhayein
            
            await query.answer("Pehle channels par request bhejein!", show_alert=True)
            return
        
        # --- FSUB CHECK KHATAM ---
        
        # Agar user ne request bhej di hai, toh file bhej dein
        ...
        # Purana FSub message delete kar dein
        await query.message.delete()
        ...
        await query.answer("✅ Success! Aapko file bhej di gayi hai.")
...
async def auto_filter(client, msg, spoll=False):
    ...
    # FIX: Add 'Refer & Earn' Button
    btn.append(
        [InlineKeyboardButton("💰 ʀᴇꜰᴇʀ & ᴇᴀʀɴ 💰", url=f"https.me/{temp.U_NAME}?start=get_referral_{message.chat.id}")]
    )
