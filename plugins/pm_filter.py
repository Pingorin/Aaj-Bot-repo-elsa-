...
import pyrogram
from info import (
    MAX_BTN, BIN_CHANNEL, USERNAME, URL, ADMINS, LANGUAGES, AUTH_CHANNEL, SUPPORT_GROUP, IMDB, IMDB_TEMPLATE, 
    LOG_CHANNEL, LOG_VR_CHANNEL, TUTORIAL, FILE_CAPTION, SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, 
    SHORTENER_API2, IS_PM_SEARCH, QR_CODE, DELETE_TIME, 
    # Import new FSub variables
    AUTH_CHANNEL_LINK, REQ_FSUB_ID_2, REQ_FSUB_LINK_2, FSUB_ID_3, FSUB_LINK_3
)
...
async def next_page(bot, query):
    ...
    # MODIFIED SECTION: Added support button and split the row
    btn.insert(0,[
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        # -------------------- NEW 3-CHANNEL FSUB CHECK --------------------
        user_id = query.from_user.id
        
        # {Channel_ID: "Invite Link"}
        fsub_channels = {
            AUTH_CHANNEL: AUTH_CHANNEL_LINK,
            REQ_FSUB_ID_2: REQ_FSUB_LINK_2,
            FSUB_ID_3: FSUB_LINK_3
        }
        ...
        for channel_id, link in fsub_channels.items():
            if not channel_id or not link:
                continue  # Skip if channel ID or Link is not set in info.py
            ...
            try:
                ...
                if member.status not in [enums.ChatMemberStatus.MEMBER, enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER]:
                    ...
                    if channel_count == 1:
                        btn_text = "Join Request Channel 1"
                    elif channel_count == 2:
                        btn_text = "Join Request Channel 2"
                    else:
                        btn_text = "Join Channel 3"
                    join_buttons.append([InlineKeyboardButton(btn_text, url=link)])
                ...
                # Don't block user if bot has an issue with a channel
                continue 

        if not subscribed_all:
            join_buttons.append([InlineKeyboardButton("🔄 Try Again 🔄", callback_data=query.data)])
            await query.answer("You must join all required channels to get files.", show_alert=True)
            await query.message.reply_text(
                "<b>Please join the following channels to get your files:</b>",
        ...
        # User is subscribed, proceed to send file
...
async def auto_filter(client, msg, spoll=False):
    ...
    #_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#
    # MODIFIED SECTION: Added support button and split/combined rows based on button count
    if offset != "":
        if total_results >= 3:
            btn.insert(0,[
                InlineKeyboardButton("♻️ sᴇɴᴅ ᴀʟʟ", callback_data=batch_link),
