...
import string
import pytz
import urllib.parse  # <-- Referral ke liye import
...
from info import (
    ...
    AUTH_CHANNEL, REQ_AUTH_CHANNEL,  # <-- Dono Fsub channels
...
# Logger ko initialize kiya gaya
...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith("get_referral_"):
        try:
            ...
            # Sahi group ke liye Referral link check karein
    ...
    # --- YAHAN SE NAYA COMBINED F-SUB BLOCK SHURU HOTA HAI ---
    ...
    if not auth_subscribed or not req_auth_subscribed:
        btn = []
        text = "🙁 <b>File lene ke liye kripya neeche diye gaye channel(s) ko join karein...</b>\n\n"
        
        # Agar AUTH_CHANNEL join nahi kiya hai to uska button add karein
        ...
        # Agar REQ_AUTH_CHANNEL join nahi kiya hai to uska button add karein
        ...
        # 'Try Again' button add karein (Link mode fix ke saath)
        ...
        return
    # --- COMBINED F-SUB BLOCK YAHAN KHATAM HOTA HAI ---
...
# NAYA DIRECT LINK REFERRAL HANDLER (RE-JOIN FIX KE SAATH)
