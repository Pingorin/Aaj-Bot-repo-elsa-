...
from pyrogram.errors import ChatAdminRequired, FloodWait, UserIsBlocked, MessageTooLong # Added MessageTooLong
...
async def start(client: Client, message):
    ...
    try:
        if data.startswith("allfiles_"):
            ...
            if not grp_id:
                # Try getting grp_id from the first file object if available
                # This depends on how files_to_send is structured; assuming it contains objects with a chat_id attr or similar
                # If not reliable, consider adding grp_id to the batch key or link
                # Example fallback (adjust based on your file object structure):
                # if files_to_send and hasattr(files_to_send[0], 'chat_id'):
                #     grp_id = files_to_send[0].chat_id
                # else:
    ...
    # ADDED LOGGING HERE
    logger.info(f"Checking CH2 for User {user_id}:")
    logger.info(f"  JOIN_REQUEST_FSUB_2 Enabled: {JOIN_REQUEST_FSUB_2}")
    logger.info(f"  AUTH_CHANNEL_2 Set: {AUTH_CHANNEL_2}")
    logger.info(f"  Has Premium: {await db.has_premium_access(user_id)}")
    logger.info(f"  Has Referral Access: {await db.check_referral_access(user_id)}")
    logger.info(f"  Has Requested Join 2: {await db.has_requested_join_2(user_id)}")

    if (JOIN_REQUEST_FSUB_2 and AUTH_CHANNEL_2 and # Check second flag and channel ID
        not await db.has_premium_access(user_id) and
        not await db.check_referral_access(user_id) and
        not await db.has_requested_join_2(user_id)): # Check second request status
        logger.info(f"Showing CH2 Button for User {user_id}") # Log when showing button
        try:
        ...
        logger.info(f"Skipping CH2 Button for User {user_id}") # Log when skipping
    ...
    if is_batch:
         ...
         for file_obj in files_to_send:
              ...
              f_caption = CAPTION.format(
                   file_name = getattr(file_obj, 'file_name', 'N/A'), # Use getattr for safety
                   file_size = get_size(getattr(file_obj, 'file_size', 0)),
                   file_caption= getattr(file_obj, 'caption', "") if getattr(file_obj, 'caption', "") else ""
              )
              f_id = getattr(file_obj, 'file_id', None)
              if not f_id:
                   logger.warning("Skipping file object in batch due to missing file_id")
                   failed_count += 1
                   continue
              ...
              try:
                   ...
                   try: # Retry
                        ...
                        logger.error(f"Failed retry sending batch file {f_id} to {user_id}: {retry_e}")
                        failed_count += 1
              except Exception as send_error:
                   logger.error(f"Failed sending batch file {f_id} to {user_id}: {send_error}")
                   failed_count += 1
        ...
        try:
             ...
             try: # Retry
                  ...
                  await message.reply_text("<b>❌ Error sending file after wait. Try again.</b>")
        except Exception as send_error:
...
async def delete(bot, message):
    ...
    if not media:
         if reply.service: await msg.edit('Cannot delete service messages.')
         else: await msg.edit('<b>Reply to a message containing the file to delete.</b>')
         return
...
async def settings_cmd(client, message):
    user_id = getattr(message.from_user, 'id', None) # Safer way to get user_id
    if not user_id:
    ...
    if not await is_check_admin(client, grp_id, user_id): # Use fetched user_id
...
async def save_template(client, message):
    ...
    try: template = message.text.split(" ", 1)[1]
    except IndexError: return await message.reply_text("<b>Usage: /set_template {template}</b>\nKeywords: {title}, {year}, {rating}, etc.")
    await save_group_settings(grp_id, 'template', template)
    await message.reply_text(f"✅ IMDB template updated for {title}.", disable_web_page_preview=True)

@Client.on_message(filters.command("send") & filters.user(ADMINS))
async def send_msg(bot, message):
    if not message.reply_to_message: return await message.reply_text("<b>Reply to message. Usage: /send id1 [id2...]</b>")
    ...
    if invalid_ids: await message.reply_text(f"⚠️ Invalid IDs skipped: `{', '.join(invalid_ids)}`")
    if not target_ids: return await message.reply_text("<b>No valid IDs provided.</b>")
    out, success_count, fail_count = "Sending Report:\n\n", 0, 0
    ...
    final_report = f"<b><u>✉️ Send Report</u></b>\n⏱️ Time: {time_taken}\nTotal: `{len(target_ids)}` | ✅ Success: `{success_count}` | ❌ Failed: `{fail_count}`\n\n" + out
    try: await message.reply_text(final_report)
    except MessageTooLong:
        with open("send_report.txt", "w") as f: f.write(re.sub('<.*?>', '', final_report))
...
@Client.on_message(filters.regex(r"^#request\s+.+") & filters.group)
async def send_request(bot, message):
    ...
    if len(request) < 3: return await message.reply_text("<b>‼️ Request too short. Example: `#request Movie (Year)`</b>")
    ...
    try:
        ...
        await message.reply_text("<b>✅ Request submitted! Wait for processing.</b>", reply_markup=InlineKeyboardMarkup(btn))
    except Exception as e:
        logger.error(f"Failed sending request to {REQUEST_CHANNEL}: {e}")
...
async def search_files_cmd(bot, message):
    ...
    file_list_text = f"🔍 Files for '{keyword}' ({total}):\n\n" + "\n".join([f"{i+1}. `{f.file_name}`" for i, f in enumerate(files)])
    try: await message.reply_text(file_list_text)
    except MessageTooLong:
        with open("search_results.txt", "w") as file:
            file_data = f"Files for '{keyword}' ({total}):\n\n" + "\n".join([f"{i+1}. {f.file_name} (ID: {f.file_id})" for i, f in enumerate(files)])
...
async def deletemultiplefiles(bot, message):
    ...
    btn = [[ InlineKeyboardButton("YES, Delete Them! ✅", callback_data=f"killfilesak#{keyword}") ], [ InlineKeyboardButton("CANCEL 😢", callback_data="close_data") ]]
...
async def save_caption(client, message):
    ...
    await message.reply_text(f"✅ Caption updated for {message.chat.title}.", disable_web_page_preview=True)
...
async def save_tutorial(client, message):
    ...
    except ValueError: return await message.reply_text("<b>Invalid URL (must start http/https).</b>")
    await save_group_settings(grp_id, 'tutorial', tutorial)
    await message.reply_text(f"<b>✅ Tutorial link updated for {message.chat.title}.</b>", disable_web_page_preview=True)
...
async def set_shortner(c, m):
    ...
    URL, API = m.command[1].strip().rstrip('/'), m.command[2].strip()
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(test_url, timeout=10) as r:
                if r.status != 200: raise Exception(f"Status: {r.status}")
                data = await r.json()
                if data.get('status')=='success' and data.get('shortenedUrl'):
                    await save_group_settings(grp_id, 'shortner', URL); await save_group_settings(grp_id, 'api', API)
                    await sts.edit(f"<b><u>✅ 1st Shortener Set!</u>\nSite: `{URL}`\nDemo: {data['shortenedUrl']}</b>", disable_web_page_preview=True)
                    try: # Log
                        u = f"@{m.from_user.username}" if m.from_user.username else m.from_user.mention
                        link = await c.export_chat_invite_link(m.chat.id)
                        g = f"[{m.chat.title}]({link})" if link else m.chat.title
                        log = f"#Shortner1\nUser:{u}(`{m.from_user.id}`)\nGrp:{g}(`{grp_id}`)\nDomain:`{URL}`"
                        await c.send_message(LOG_API_CHANNEL, log, disable_web_page_preview=True)
                    except Exception as le: logger.error(f"S1 log err: {le}")
                else:
                    err = data.get('message', 'API Error'); await sts.edit(f"<b><u>❌ API Fail!</u> `{err}`\nDefault restored.</b>")
                    await save_group_settings(grp_id, 'shortner', SHORTENER_WEBSITE); await save_group_settings(grp_id, 'api', SHORTENER_API)
    except asyncio.TimeoutError:
         await sts.edit(f"<b><u>❌ Timeout!</u> `{URL}`\nDefault restored.</b>"); await save_group_settings(grp_id, 'shortner', SHORTENER_WEBSITE); await save_group_settings(grp_id, 'api', SHORTENER_API)
    except Exception as e:
        logger.error(f"Set S1 err: {e}"); await sts.edit(f"<b><u>❌ Error!</u> `{e}`\nDefault restored.</b>"); await save_group_settings(grp_id, 'shortner', SHORTENER_WEBSITE); await save_group_settings(grp_id, 'api', SHORTENER_API)

@Client.on_message(filters.command('set_shortner_2') & filters.group)
async def set_shortner_2(c, m):
    grp_id = m.chat.id
    if not await is_check_admin(c, grp_id, m.from_user.id): return await m.reply_text('<b>Group admins only.</b>')
    if len(m.command) != 3: return await m.reply("<b>Usage: /set_shortner_2 site2.com api_key2</b>")
    sts = await m.reply("<b>♻️ Checking API...</b>")
    URL, API = m.command[1].strip().rstrip('/'), m.command[2].strip()
    if '.' not in URL or '/' in URL.split('//', 1)[-1]: return await sts.edit("<b>⚠️ Invalid URL (domain only).</b>")
    test_url = f"https://{URL}/api?api={API}&url=https://telegram.dog"
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(test_url, timeout=10) as r:
                ...
                data = await r.json()
                if data.get('status')=='success' and data.get('shortenedUrl'):
                    await save_group_settings(grp_id, 'shortner_two', URL); await save_group_settings(grp_id, 'api_two', API)
                    ...
                    try: # Log
                        ...
                        log = f"#Shortner2\nUser:{u}(`{m.from_user.id}`)\nGrp:{g}(`{grp_id}`)\nDomain:`{URL}`"
                        await c.send_message(LOG_API_CHANNEL, log, disable_web_page_preview=True)
                    except Exception as le: logger.error(f"S2 log err: {le}")
                else:
                    err = data.get('message', 'API Error'); await sts.edit(f"<b><u>❌ API Fail!</u> `{err}`\nDefault restored.</b>")
                    await save_group_settings(grp_id, 'shortner_two', SHORTENER_WEBSITE2); await save_group_settings(grp_id, 'api_two', SHORTENER_API2)
    except asyncio.TimeoutError:
         await sts.edit(f"<b><u>❌ Timeout!</u> `{URL}`\nDefault restored.</b>"); await save_group_settings(grp_id, 'shortner_two', SHORTENER_WEBSITE2); await save_group_settings(grp_id, 'api_two', SHORTENER_API2)
    except Exception as e:
        logger.error(f"Set S2 err: {e}"); await sts.edit(f"<b><u>❌ Error!</u> `{e}`\nDefault restored.</b>"); await save_group_settings(grp_id, 'shortner_two', SHORTENER_WEBSITE2); await save_group_settings(grp_id, 'api_two', SHORTENER_API2)
...
async def set_log(client, message):
    ...
    try:
        log_ch_id = int(message.command[1])
        if not str(log_ch_id).startswith("-100"): raise ValueError("ID format")
    except ValueError: return await message.reply_text('<b>Invalid ID (-100...).</b>')
    sts = await message.reply("<b>♻️ Checking access...</b>")
    try:
       test = await client.send_message(log_ch_id, "<b>Testing...</b>"); await asyncio.sleep(1); await test.delete()
    except Exception as e: return await sts.edit(f'<b>❌ Error `{log_ch_id}`.\nBot admin?\nErr: `{e}`</b>')
    await save_group_settings(grp_id, 'log', log_ch_id)
    await sts.edit(f"<b>✅ Log Channel set for {message.chat.title} to `{log_ch_id}`.</b>")
    try: # Log
        u = f"@{message.from_user.username}" if message.from_user.username else message.from_user.mention
        link = await client.export_chat_invite_link(message.chat.id)
        g = f"[{message.chat.title}]({link})" if link else message.chat.title
        log = f"#LogChannel\nUser:{u}(`{message.from_user.id}`)\nGrp:{g}(`{grp_id}`)\nLog CH:`{log_ch_id}`"
        await client.send_message(LOG_API_CHANNEL, log, disable_web_page_preview=True)
    except Exception as le: logger.error(f"LogCh set log err: {le}")
...
async def all_settings(client, message):
    ...
    if not await is_check_admin(client, grp_id, message.from_user.id): return await message.reply_text('<b>Group admins only.</b>')
    settings = await get_settings(grp_id)
    text = f"""<b><u>⚙️ Settings For:</u> {message.chat.title}</b> ({grp_id})
...
2nd Gap: {get_readable_time(settings.get('verify_time', TWO_VERIFY_GAP))}

<b>Channels & Links:</b>
Log CH: <code>{settings.get('log', 'N/A')}</code>
Tutorial: {settings.get('tutorial', 'N/A')}

<b>Formatting:</b>
IMDB Template: (use /get_template)
File Caption: (use /get_caption)
"""
...
async def get_format_cmd(client, message):
    ...
    key = 'template' if message.command[0] == 'get_template' else 'caption'
...
async def set_time(client, message):
    ...
    try:
        ...
        time_sec = await get_seconds(time_str)
        if time_sec <= 0: raise ValueError("Positive time")
    ...
    await save_group_settings(message.chat.id, 'verify_time', time_sec)
    await message.reply_text(f"✅ 2nd verify gap set for {message.chat.title} to <b>{get_readable_time(time_sec)}</b>.")

@Client.on_callback_query(filters.regex('^open_settings_panel'))
async def open_settings_panel_callback(client, query):
     if not await is_check_admin(client, query.message.chat.id, query.from_user.id): return await query.answer("Only admins.", show_alert=True)
     await query.answer("Opening settings panel...")
     dummy_msg = query.message
     dummy_msg.from_user = query.from_user
     await settings_cmd(client, dummy_msg)
