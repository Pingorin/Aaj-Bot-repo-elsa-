...
import string
import pytz
import urllib.parse  # <-- Referral ke liye add kiya gaya
from datetime import datetime, timedelta
from Script import script
from pyrogram import Client, filters, enums
# UserNotParticipant ko FSub check ke liye import kiya gaya
from pyrogram.errors import ChatAdminRequired, FloodWait, UserIsBlocked, PeerIdInvalid, UserNotParticipant 
...
from info import (
    ...
    # AUTH_CHANNEL hata diya gaya hai
    ...
    REQ_SUB_CHANNEL_1, REQ_SUB_CHANNEL_2  # <-- Naye FSub channels import kiye gaye
)
# 'is_req_subscribed' ko utils import se hata diya gaya hai
from utils import get_settings, save_group_settings, get_size, get_shortlink, is_check_admin, get_status, temp, get_readable_time
import re
import json
import base64
...
async def start(client:Client, message): 
    ...
    # --- NAYA FSUB CHECK (Member OR Pending Request) ---
    user_id = message.from_user.id
    
    # --- Helper function membership check karne ke liye ---
    async def get_membership_status(channel_id, user_id):
        if not channel_id:
            return "ok"  # Channel set nahi hai, matlab "pass"
        try:
            member = await client.get_chat_member(channel_id, user_id)
            if member.status in [enums.ChatMemberStatus.MEMBER, enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER]:
                return "ok"  # User channel mein hai
            else:
                return "not_in" # User BANNED ya LEFT hai
        except UserNotParticipant:
            return "not_in" # User channel mein nahi hai (ya chhod diya hai)
        except Exception as e:
            logger.error(f"FSub check ke liye get_chat_member fail hua (Channel: {channel_id}): {e}")
            return "error" # Bot ko permission nahi hai

    # --- Dono channels ka status check karein ---
    try:
        status_1 = await get_membership_status(REQ_SUB_CHANNEL_1, user_id)
        status_2 = await get_membership_status(REQ_SUB_CHANNEL_2, user_id)
    except ChatAdminRequired:
        await message.reply_text("Bot configuration mein error hai. Bot ko FSub channels mein admin banayein.")
        return

    if status_1 == "error" or status_2 == "error":
        await message.reply_text("Bot configuration mein error hai (cannot check members). Kripya admin se sampark karein.")
        return

    # --- Ab 'Pending Request' check karein (agar user member nahi hai) ---
    
    # Channel 1 ki 'pass' condition
    pass_1 = False
    if status_1 == "ok":
        pass_1 = True
    elif REQ_SUB_CHANNEL_1:
        # User member nahi hai, check karein ki request pending hai ya nahi
        if await db.find_join_req(user_id, REQ_SUB_CHANNEL_1):
            pass_1 = True # Request pending hai, user ko pass karein
    else:
        pass_1 = True # Channel set nahi hai (REQ_SUB_CHANNEL_1 is None or 0)
            
    # Channel 2 ki 'pass' condition
    pass_2 = False
    if status_2 == "ok":
        pass_2 = True
    elif REQ_SUB_CHANNEL_2:
        # User member nahi hai, check karein ki request pending hai ya nahi
        if await db.find_join_req(user_id, REQ_SUB_CHANNEL_2):
            pass_2 = True # Request pending hai, user ko pass karein
    else:
        pass_2 = True # Channel set nahi hai (REQ_SUB_CHANNEL_2 is None or 0)

    # --- Final Decision ---
    
    # Agar user dono conditions pass karta hai (ya toh member hai YA request pending hai)
    if pass_1 and pass_2:
        # FSub Pass. Code neeche verification/file logic tak continue hoga
        pass
    
    else:
        # User FSub Fail. Use join karne ko bolein.
        try:
            btn = []
            # Sirf uss channel ka button dikhayein jise user ne pass nahi kiya
            if not pass_1 and REQ_SUB_CHANNEL_1:
                link_1 = await client.create_chat_invite_link(int(REQ_SUB_CHANNEL_1), creates_join_request=True)
                btn.append([InlineKeyboardButton("➡️ Channel 1 Join Karein ⬅️", url=link_1.invite_link)])
            if not pass_2 and REQ_SUB_CHANNEL_2:
                link_2 = await client.create_chat_invite_link(int(REQ_SUB_CHANNEL_2), creates_join_request=True)
        except ChatAdminRequired:
            logger.error("Bot ko dono REQ_SUB channels mein 'Invite Users' permission ke saath Admin hona chahiye.")
            await message.reply_text("Bot configuration mein error hai. Kripya admin se sampark karein.")
            return
        except Exception as e:
            logger.error(f"FSub ke liye invite link banate waqt error: {e}")
            await message.reply_text(f"Ek error aa gaya hai. Kripya admin se sampark karein.\nError: {e}")
            return
        ...
        if message.command[1] != "subscribe":
            btn.append(
                [InlineKeyboardButton(
                    "♻️ Request Bhejne Ke Baad Yahan Click Karein ♻️", 
        ...
        await client.send_message(
            ...
            text=(
                "**❌ Access Denied ❌**\n\n"
                "Aap hamare channel(s) ke member nahi hain aur na hi aapki join request pending hai.\n\n"
                "File lene ke liye pehle join request bhejein.\n\n"
                "1. Neeche diye gaye button(s) par click karein.\n"
                "2. Channel par **'Request to Join'** button dabayein.\n"
                "3. Jab request bhej dein, tab **'Request Bhejne Ke Baad Yahan Click Karein'** button dabayein."
            ),
            reply_markup=InlineKeyboardMarkup(btn),
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener:
            ...
            reply_markup=InlineKeyboardMarkup(buttons)            
            msg = script.SECOND_VERIFY_TEXT if is_second_shortener else script.VERIFICATION_TEXT
...
async def save_tutorial(client, message):
    ...
    chat_type = message.chat.type
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await message.reply_text("<b>ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ ɪN ɢʀᴏᴜᴘ...</b>")
    try:
...
# NAYA DIRECT LINK REFERRAL HANDLER (GROUP-SPECIFIC)
...
async def welcome_handler(client: Client, member: ChatMemberUpdated):
    try:
        ...
        if (
            ...
            # 2. Re-join check (Group-specific)
            # 'has_been_referred_in_group' ko users_chats_db se fix kiya gaya
            ...
            referrer_mention = f"User ({referrer_id})" # Default fallback
            try:
                referrer_user = await client.get_users(referrer_id)
                referrer_mention = referrer_user.mention
            except Exception as e:
                logger.warning(f"Could not fetch referrer user {referrer_id}: {e}")
