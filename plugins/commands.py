...
from info import (
    ...
    AUTH_CHANNEL_1, AUTH_CHANNEL_2  # Puraney 'AUTH_CHANNEL' ki jagah naye variables
)
...
# FIX: Initialized logger
logger = logging.getLogger(__name__)

# FIX: Import path ko 'plugins.Join_req' kiya gaya
try:
    from plugins.Join_req import build_fsub_check
except ImportError:
    # FIX: Error message ko bhi theek kiya gaya
    logger.error("Fsub handler (plugins/Join_req.py) file nahi mili. FSUB kaam nahi karega.")
    build_fsub_check = None # Error se bachne ke liye
...
async def start(client:Client, message): 
    ...
    # --- LOGIC RE-ORDERED: Pehle file details set karein ---
    
    data = message.command[1]
    grp_id = 0
    check_file_id = None
    is_all_files = False
    files_list = [] # file list ko store karne ke liye

    if data and data.startswith("allfiles"):
        is_all_files = True
        try:
            _, key = data.split("_", 1)
            files_list = temp.FILES_ID.get(key)
            if files_list:
                check_file_id = files_list[0].file_id
                # temp.CHAT se grp_id lein (jab user 'Send All' click karta hai)
                grp_id = temp.CHAT.get(user_id, 0)
            else:
                await message.reply_text("<b>⚠️ ꜰɪʟᴇ ʙᴀᴛᴄʜ ᴇxᴘɪʀᴇᴅ ᴏʀ ɴᴏᴛ ꜰᴏᴜɴᴅ ⚠️\n\nKripya group mein phir se search karein.</b>")
                return
        except Exception as e:
            logger.error(f"Allfiles error: {e}")
            await message.reply_text("<b>⚠️ ɪɴᴠᴀʟɪᴅ ꜰɪʟᴇ ʙᴀᴛᴄʜ ʟɪɴᴋ ⚠️</b>")
            return
        ...
        try:
            ...
            try:
                # Base64 decode try karein
                pre, file_id = ((base64.urlsafe_b64decode(data + "=" * (-len(data) % 4))).decode("ascii")).split("_", 1)
                check_file_id = file_id
                # Base64 requests mein grp_id nahi hota, temp se lein
                grp_id = temp.CHAT.get(user_id, 0)
            except:
                await message.reply('<b>⚠️ ꜰɪʟᴇ ɴᴏᴛ ꜰᴏᴜɴᴅ / ʟɪɴᴋ ᴇxᴘɪʀᴇᴅ ⚠️</b>')
                return
                
    if not check_file_id:
        await message.reply_text("<b>⚠️ ᴄᴏᴜʟᴅ ɴᴏᴛ ꜰɪɴᴅ ꜰɪʟᴇ ɪᴅ ᴛᴏ ᴄʜᴇᴄᴋ ⚠️</b>")
        return
        
    grp_id = int(grp_id) # Ensure grp_id integer hai

    # --- CHECKS AB YAHAN RUN HONGE ---
    
    fsub_status = "PASS" # Default
    fsub_message = ""
    
    if not await db.has_premium_access(user_id):
        
        # 1. NAYA ADVANCED FSUB LOGIC
        if AUTH_CHANNEL_1 and AUTH_CHANNEL_2 and build_fsub_check:
            fsub_status, fsub_message, fsub_buttons = await build_fsub_check(
                client, message.from_user.id, check_file_id, grp_id
            )
            
            if fsub_status == "FAIL":
                await message.reply_text(fsub_message, reply_markup=fsub_buttons, disable_web_page_preview=True)
                return # User fail hua, yahin ruk jaayein
        
        # 2. VERIFICATION LOGIC (Fsub pass hone ke baad)
        settings = await get_settings(grp_id)
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener:
            ...
            # check_file_id use karein
            verify = await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start=notcopy_{user_id}_{verify_id}_{check_file_id}", grp_id, is_second_shortener)
            buttons = [[
    ...
    # --- CHECKS PASSED - AB FILES BHEJEIN ---

    if is_all_files:
        settings = await get_settings(grp_id)
        for file in files_list: # Upar store ki gayi list use karein
            ...
            try:
                ...
                logger.warning(f"Failed to send file {file.file_id} to {user_id}: {e}")
        return # Kaam khatam

    # --- SINGLE FILE LOGIC ---
    files_ = await get_file_details(check_file_id)           
    if not files_:
        # Agar check_file_id yahan fail hota hai, toh yeh ek serious error hai
        # (Kyunki base64 pehle hi handle ho jaana chahiye tha)
        await message.reply('<b>⚠️ ꜰɪʟᴇ ɴᴏᴛ ꜰᴏᴜɴᴅ ɪɴ ᴅᴀᴛᴀʙᴀsᴇ ⚠️</b>')
        return
            
    files = files_[0]
    settings = await get_settings(grp_id)
    
    # FSUB MESSAGE KO CAPTION MEIN JODEIN
    final_caption_message = ""
    if fsub_status in ["PENDING", "MEMBER"]: 
        final_caption_message = f"**{fsub_message}**\n\n"
    ...
    final_caption = final_caption_message + f_caption

    btn = [[
        InlineKeyboardButton("✛ ᴡᴀᴛᴄʜ & ᴅᴏᴡɴʟᴏᴀᴅ ✛", callback_data=f'stream#{check_file_id}')
    ]]
...
async def delete(bot, message):
    if message.from_user.id not in ADMINS:
        await message.reply('ᴏɴʟY ᴛʜᴇ ʙᴏᴛ ᴏᴡɴᴇʀ ᴄᴀɴ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ... 😑')
        return
...
async def delete_all_index(bot, message):
    ...
    if message.from_user.id not in ADMINS:
        await message.reply('ᴏɴʟY ᴛʜᴇ ʙᴏᴛ ᴏᴡɴᴇʀ ᴄᴀɴ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴḍ... 😑')
        return
...
async def set_shortner(c, m):
    ...
    if not await is_check_admin(c, grp_id, m.from_user.id):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪN ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')        
...
# --- NAYA FSUB "TRY AGAIN" CALLBACK HANDLER ---
...
async def check_fsub_callback(client: Client, query: CallbackQuery):
    ...
    Yeh 'start' command waale logic ko hi repeat karega.
    """
    # Callback data se grp_id aur file_id nikalein
    # Format: "check_fsub#{grp_id}#{file_id}"
    try:
        _, grp_id, file_id = query.data.split("#")
        grp_id = int(grp_id)
    except:
        await query.answer("Error: Invalid callback data. Please try sending the request again.", show_alert=True)
        return

    if not build_fsub_check:
        await query.answer("Error: FSUB system is not configured.", show_alert=True)
        return

    fsub_status, fsub_message, fsub_buttons = await build_fsub_check(
        client, query.from_user.id, file_id, grp_id
    )

    if fsub_status == "FAIL":
        await query.answer("Aap abhi bhi channels se nahi jude hain.", show_alert=True)
        await query.message.edit_text(fsub_message, reply_markup=fsub_buttons, disable_web_page_preview=True)
        return
