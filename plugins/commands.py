...
import string
import pytz
import urllib.parse  # <-- YEH IMPORT MAUJOOD HAI
...
from info import (
    ...
    REQUEST_CHANNEL, REFERRAL_TARGET, PREMIUM_MONTH_DURATION, REQ_AUTH_CHANNEL # REQ_AUTH_CHANNEL add kar diya hai
)
...
async def start(client:Client, message): 
    ...
    # --- F-SUB CHECK (AUTH_CHANNEL & REQ_AUTH_CHANNEL) ---
    # Yeh poora block yahan se HATA diya gaya hai
    # Kyunki yeh logic ab pm_filter.py mein chala gaya hai
    # Taaki yeh group search aur PM search, dono par kaam kare

    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help", "buy_premium"]:
        if message.command[1] == "buy_premium":
            btn = [[
                InlineKeyboardButton('📸 sᴇɴᴅ sᴄʀᴇᴇN_sʜᴏᴛ 📸', url=USERNAME)
            ],[
