...
# --- FIX: Imported get_fsub_channels_to_join, removed check_fsub_status ---
from utils import (
...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        if settings['log']: # Log channel set hai ya nahi check karein
            try:
                ...
                logger.error(f"Log channel mein message nahi bhej paaya: {e}")
        btn = [[
    ...
    if AUTH_CHANNEL or AUTH_CHANNEL_2:
        ...
        # Agar list khaali nahi hai, iska matlab user ko channel join karna hai
        if len(channels_to_join) > 0:
            ...
            for i, chat_id in enumerate(channels_to_join):
                try:
                    ...
                    logger.error(f"Bot ko {chat_id} mein admin (invite link permission) banayein")
                    continue # Agar bot admin nahi hai toh uss button ko skip kar do
                except Exception as e:
                    logger.error(e)
                    continue
        ...
        # Agar list khaali hai (yaani user sab channels mein member/pending hai)
        # Toh check karo ki file request thi aur "Pending" message dikhana hai ya nahi
        if len(message.command) > 1 and message.command[1].startswith("file_"):
            is_pending = False
            # --- FIX: Added 'is not None' check ---
            if AUTH_CHANNEL is not None:
                is_pending = await db.is_join_request_pending(message.from_user.id, AUTH_CHANNEL)
            
            # --- FIX: Added 'is not None' check ---
            if not is_pending and AUTH_CHANNEL_2 is not None: # Agar pehle mein pending nahi, toh doosra check karo
                 is_pending = await db.is_join_request_pending(message.from_user.id, AUTH_CHANNEL_2)
    ...
    # --- PURANA FSUB BLOCK (DELETED) ---
    # Yeh poora block (Line 228-250) yahan se hata diya gaya hai kyunki yeh naye logic se replace ho gaya hai
    # aur ismein 'is_req_subscribed' function tha jo ab exist nahi karta.
    # --- BLOCK DELETED ---
