...
from info import (
    ...
    AUTH_CHANNEL, AUTH_CHANNEL_2, # <-- FIX: Added AUTH_CHANNEL_2
...
# --- FIX: Imported get_fsub_channels_to_join, removed old functions ---
from utils import (
    get_settings, save_group_settings, get_fsub_channels_to_join, get_size, 
...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        # --- FIX: Added check for settings['log'] ---
        if settings['log']: # Log channel set hai ya nahi check karein
            try:
                await client.send_message(settings['log'], script.VERIFIED_LOG_TEXT.format(m.from_user.mention, user_id, datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%d %B %Y'), num))
            except Exception as e:
                logger.error(f"Log channel mein message nahi bhej paaya (User {user_id}): {e}")
    ...
    # --- NEW ADVANCED DUAL FSUB LOGIC ---
    if AUTH_CHANNEL or AUTH_CHANNEL_2:
        # Naya function call karein jo list return karta hai
        channels_to_join = await get_fsub_channels_to_join(client, message.from_user.id)

        # Case 3: Agar list khaali nahi hai, iska matlab user ko channel join karna hai
        if len(channels_to_join) > 0:
            
            btn = [] # Buttons ki list
            channel_names = [] # Messages ke liye channel naam
            
            # Har channel ke liye ek button banayein jo join karna baaki hai
            for i, chat_id in enumerate(channels_to_join):
                try:
                    chat = await client.get_chat(chat_id)
                    invite_link = await client.create_chat_invite_link(chat_id, creates_join_request=True)
                    
                    channel_names.append(f"**{chat.title}**")
                    btn.append(
                        [InlineKeyboardButton(f"Request to Join {chat.title}", url=invite_link.invite_link)]
                    )
                
                except ChatAdminRequired:
                    # --- FIX: Added detailed error handling for Admins ---
                    logger.error(f"CRITICAL: Bot ko {chat_id} mein admin (can_invite_users permission) banayein")
                    if message.from_user.id in ADMINS:
                        await message.reply_text(
                            f"**FSUB ERROR:** Bot ko channel (`{chat_id}`) mein Admin banayein aur **'Invite users'** permission dein.\n\n"
                            "Uske bina main 'Request to Join' link nahi bana sakta."
                        )
                    continue # Iss button ko skip karke aage badho
                
                except Exception as e:
                    logger.error(f"Join link create karne mein error ({chat_id}): {e}")
                    if message.from_user.id in ADMINS:
                        await message.reply_text(
                            f"**FSUB ERROR:** Channel (`{chat_id}`) ke liye link banate waqt error aaya.\n\n"
                            f"`{e}`\n\n"
                            "**Check karein ki channel PRIVATE hai aur 'Approve New Members' ON hai.**"
                        )
                    continue # Iss button ko skip karke aage badho

            # Agar koi valid button bana hai tabhi message bhejein
            if btn:
                # "Try Again" button
                ...
                await message.reply_text(
                    f"**File lene ke liye, aapko {', '.join(channel_names)} channel join karne ki request bhejni hogi.**\n\n"
        ...
        # Case 1 & 2: Agar list khaali hai (yaani user sab channels mein member/pending hai)
        # Check karo ki file request thi aur "Pending" message dikhana hai ya nahi
        if len(message.command) > 1 and message.command[1].startswith("file_"):
            
            is_pending_any = False
            # --- FIX: Check if channels are not None before checking pending status ---
            if AUTH_CHANNEL is not None:
                if await db.is_join_request_pending(message.from_user.id, AUTH_CHANNEL):
                    is_pending_any = True
            
            if not is_pending_any and AUTH_CHANNEL_2 is not None:
                 if await db.is_join_request_pending(message.from_user.id, AUTH_CHANNEL_2):
                    is_pending_any = True
    ...
    # --- FIX: DELETED old Fsub block that used 'is_req_subscribed' ---
    # Poora `if AUTH_CHANNEL and not await is_req_subscribed...` block yahan se hata diya gaya hai.
    # ---
...
async def set_shortner_2(c, m):
    ...
    try:
        ...
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https->sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner_2 mdiskshortner.link e7beb3c8f756dfa15d0bec495abc65f58c0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)
