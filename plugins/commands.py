...
from info import (
    ...
    REQ_SUB_CHANNEL_1, REQ_SUB_CHANNEL_2  # <-- NAYE FSUB CHANNELS IMPORT KIYE HAIN
)
...
async def start(client:Client, message): 
    ...
    if not has_req_1 or not has_req_2:
        ...
        # "Try Again" button add karein (purane AUTH_CHANNEL block se logic copy kiya gaya)
        if message.command[1] != "subscribe":
            try:
                kk, grp_id, file_id = message.command[1].split('_', 2)
                pre = 'checksubp' if kk == 'filep' else 'checksub'
                # Note: Naye system mein 'checksub' callback ki zaroorat nahi honi chahiye,
                # lekin hum purane code se compatibility ke liye ise rakhein
                btn.append(
        ...
        await client.send_message(
            ...
            text=(
                ...
                "3. Jab dono channels par request bhej dein, tab **'Try Again'** button dabayein."
            ),
            reply_markup=InlineKeyboardMarkup(btn),
...
# NAYA DIRECT LINK REFERRAL HANDLER (ATTRIBUTEERROR FIX KE SAATH)
