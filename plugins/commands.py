...
async def start(client:Client, message): 
    ...
    # --- NAYA FSUB CHECK (Member OR Pending Request) V2 (CLEANED) ---
    ...
    # --- Naya Logic ---
    show_fsub_message = False
    fsub_buttons = []

    # --- Channel 1 ko check karein ---
    if REQ_SUB_CHANNEL_1:
        status_1 = await get_membership_status(REQ_SUB_CHANNEL_1, user_id)
        
        if status_1 == "error":
            await message.reply_text("Bot configuration error (Ch 1). Kripya admin se sampark karein.")
            return
            
        if status_1 == "not_in":
            # User member nahi hai, ab pending request check karein
            if not await db.find_join_req(user_id, REQ_SUB_CHANNEL_1):
                # User member bhi nahi hai AUR request pending bhi nahi hai
                show_fsub_message = True
                try:
                    link_1 = await client.create_chat_invite_link(int(REQ_SUB_CHANNEL_1), creates_join_request=True)
                    fsub_buttons.append([InlineKeyboardButton("➡️ Channel 1 Join Karein ⬅️", url=link_1.invite_link)])
                except Exception as e:
                    logger.error(f"FSub link 1 banate waqt error: {e}")
                    await message.reply_text(f"Channel 1 ka link banate waqt error. Admin se sampark karein.\nError: {e}")
                    return

    # --- Channel 2 ko check karein ---
    if REQ_SUB_CHANNEL_2:
        status_2 = await get_membership_status(REQ_SUB_CHANNEL_2, user_id)

        if status_2 == "error":
            await message.reply_text("Bot configuration error (Ch 2). Kripya admin se sampark karein.")
            return

        if status_2 == "not_in":
            # User member nahi hai, ab pending request check karein
            if not await db.find_join_req(user_id, REQ_SUB_CHANNEL_2):
                # User member bhi nahi hai AUR request pending bhi nahi hai
                show_fsub_message = True
                try:
                    link_2 = await client.create_chat_invite_link(int(REQ_SUB_CHANNEL_2), creates_join_request=True)
                    fsub_buttons.append([InlineKeyboardButton("➡️ Channel 2 Join Karein ⬅️", url=link_2.invite_link)])
                except Exception as e:
                    logger.error(f"FSub link 2 banate waqt error: {e}")
                    await message.reply_text(f"Channel 2 ka link banate waqt error. Admin se sampark karein.\nError: {e}")
                    return

    # --- Final Decision ---
    if show_fsub_message:
        # User ko FSub message dikhayein
    ...
    # --- FSUB CHECK KHATAM ---
    # Agar code yahan tak pahunchta hai, iska matlab user ne FSub pass kar liya hai
    # --- (Ab verification aur file logic continue hogi) ---
...
async def send_request(bot, message):
    ...
    buttons = [[
        ...
        InlineKeyboardButton('⚙ sʜᴏᴡ ᴏᴘᴛɪᴏN ⚙', callback_data=f'show_options#{message.from_user.id}#{message.id}')
    ]]
