...
async def start(client:Client, message): 
    ...
    # --- NAYA REFERRAL CODE BLOCK (Aapki file se missing tha) --
    ...
    # User ka permanent FSub status check karein
    if not await db.get_fsub_status(user_id):
        # User permanently approved nahi hai, ab 'join request' logs check karein
        ...
        if not has_req_1 or not has_req_2:
            ...
            # User ne 'join request' check ko pass kar liya hai
            # Usey permanent 'fsub_passed' = True mark kar dein
            await db.update_fsub_status(user_id, True)
