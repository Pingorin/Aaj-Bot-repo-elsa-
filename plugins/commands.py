...
# MODIFIED: Added MediaSecondary and get_files_db_size for the stats command
from database.ia_filterdb import Media, get_file_details, get_bad_files, unpack_new_file_id, MediaSecondary, get_files_db_size
from database.users_chats_db import db
from info import ADMINS, LOG_CHANNEL, USERNAME, VERIFY_IMG, IS_VERIFY, FILE_CAPTION, AUTH_CHANNEL, AUTH_CHANNEL_2, SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, SHORTENER_API2, LOG_API_CHANNEL, TWO_VERIFY_GAP, QR_CODE, DELETE_TIME
...
async def start(client:Client, message): 
    ...
    # -------- NEW JOIN REQUEST FSUB LOGIC --------
    if AUTH_CHANNEL or AUTH_CHANNEL_2:
        channels_to_join = []
        ...
        async def check_channel(channel_id):
            ...
            try:
                ...
                logging.error(f"Bot is not an admin in FSub channel: {channel_id}")
            except Exception as e:
                logging.error(f"Error checking membership for {message.from_user.id} in {channel_id}: {e}")

        await check_channel(AUTH_CHANNEL)
        await check_channel(AUTH_CHANNEL_2)
        ...
        if channels_to_join:
            ...
            for channel_id in channels_to_join:
                try:
                    ...
                    btn.append([InlineKeyboardButton(f"➡️ Request to Join {channel_info.title} ⬅️", url=invite_link.invite_link)])
                except Exception as e:
                    logging.error(f"Could not create invite link for {channel_id}: {e}")
                    continue
            
            if not btn:
                await message.reply("Sorry, there was an error setting up the join links. Please contact an admin.")
                return

            btn.append([InlineKeyboardButton("♻️ Try Again ♻️", url=f"https://t.me/{temp.U_NAME}?start={data}")])
            
            await message.reply(
                "**You must first send a request to join our channel(s) to get files.**\n\n"
                "Click the button(s) below to send your request. Once an admin approves it, come back here and click **'Try Again'**.",
                reply_markup=InlineKeyboardMarkup(btn)
            )
            return
    # -------- END OF FSUB LOGIC --------
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener:
            ...
            try:
                verify_url = await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start=notcopy_{user_id}_{verify_id}_{file_id}", grp_id, is_second_shortener)

                if not verify_url or not verify_url.startswith(('http://', 'https://')):
                    logging.error(f"Generated an invalid verification URL: {verify_url}")
                    await m.reply_text("⚠️ **Verification link could not be created.** The shortener service might be down. Please try again later.")
                    return

                first_row = [InlineKeyboardButton(text="✅️ ᴠᴇʀɪғʏ ✅️", url=verify_url)]

                tutorial_url = settings.get('tutorial')
                if tutorial_url and tutorial_url.startswith(('http://', 'https://')):
                    first_row.append(InlineKeyboardButton(text="⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url=tutorial_url))
                ...
                logging.error(f"Error during verification link generation: {e}", exc_info=True)
                await m.reply_text("An unexpected error occurred. Please contact support.")
...
async def stats(client: Client, message):
    msg = await message.reply("Fetching stats... ⏳")
    try:
        total_users = await db.total_users_count()
        total_chats = await db.total_chat_count()
        
        # Get file counts from both databases
        total_files_primary = await Media.count_documents()
        total_files_secondary = await MediaSecondary.count_documents()
        total_files = total_files_primary + total_files_secondary
        
        # Get DB sizes
        users_db_size_bytes = await db.get_db_size()
        files_db_size_primary_bytes = await get_files_db_size('primary')
        files_db_size_secondary_bytes = await get_files_db_size('secondary')
        
        total_db_size_bytes = users_db_size_bytes + files_db_size_primary_bytes + files_db_size_secondary_bytes
        
        # Format sizes
        users_db_size = get_size(users_db_size_bytes)
        files_db_size_primary = get_size(files_db_size_primary_bytes)
        files_db_size_secondary = get_size(files_db_size_secondary_bytes)
        total_db_size = get_size(total_db_size_bytes)

        text = f"""<b>📊 BOT STATISTICS 📊</b>

<b><u>Users:</u></b>
<b>👥 Total Users:</b> <code>{total_users}</code>
<b>🏘️ Total Chats:</b> <code>{total_chats}</code>

<b><u>Files (Dual DB):</u></b>
<b>🗂️ Primary DB:</b> <code>{total_files_primary}</code> files
<b>🗂️ Secondary DB:</b> <code>{total_files_secondary}</code> files
<b>📂 Total Files:</b> <code>{total_files}</code>

<b><u>Database Size:</u></b>
<b>🗄️ Users DB:</b> <code>{users_db_size}</code>
<b>🗄️ Files DB (Primary):</b> <code>{files_db_size_primary}</code>
<b>🗄️ Files DB (Secondary):</b> <code>{files_db_size_secondary}</code>
<b>💾 Total DB Size:</b> <code>{total_db_size}</code>
"""
...
async def send_msg(bot, message):
    ...
    if message.reply_to_message:
        ...
        await message.reply_text("<b>ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴդ ᴀꜱ ᴀ ʀᴇᴘʟʏ ᴛᴏ ᴀɴʏ ᴍᴇꜱꜱᴀɢᴇ, ꜰᴏʀ ᴇɢ - <code>/send userid1 userid2</code></b>")
...
async def set_log(client, message):
    ...
    user_id = message.from_user.id # Corrected: 'm' was not defined here, using 'message'
