...
from info import (
    ...
    AUTH_CHANNEL, AUTH_CHANNEL_2, # <-- FIX: Naya FSub channel import kiya
...
# <-- FIX: Naya FSub function 'check_fsub' import kiya
...
async def start(client:Client, message): 
    ...
    # --- FIX: YAHAN SE NAYA ADVANCED FSUB LOGIC SHURU HOTA HAI ---
    if AUTH_CHANNEL or AUTH_CHANNEL_2:
        fsub_pass, failed_channels_list = await check_fsub(client, message)
        
        if not fsub_pass:
            # User check fail kar gaya
            btn = [] # Naya button list
            
            # Har failed channel ke liye ek-ek karke link banayein
            for channel_id in failed_channels_list:
                try:
                    ...
                    # Button ko behtar dikhane ke liye naam set karein
                    channel_name = "Channel 1"
                    if channel_id == AUTH_CHANNEL_2:
                        channel_name = "Channel 2" # Ise aap info.py se bhi la sakte hain
                    
                    # Button ko list mein add karein
                    btn.append(
                        [InlineKeyboardButton(f"⛔️ ᴊᴏɪɴ {channel_name} ⛔️", url=invite_link.invite_link)]
                    )
                except ChatAdminRequired:
                    logger.error(f"Make sure Bot is admin in FSub channel: {channel_id}")
                except Exception as e:
                    logger.error(f"Could not create invite link for {channel_id}: {e}")
                    await message.reply(f"Error: <code>{channel_id}</code> ke liye invite link nahi ban pa raha hai. Admin se contact karein.")
                    return

            # Try Again button ko add karein
