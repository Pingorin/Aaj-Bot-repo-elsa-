...
from info import (
    ...
    SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, # <-- FIX: AUTH_CHANNEL YAHAN SE HATA DIYA
    SHORTENER_API2, LOG_API_CHANNEL, TWO_VERIFY_GAP, QR_CODE, DELETE_TIME, 
    REQUEST_CHANNEL, REFERRAL_TARGET, PREMIUM_MONTH_DURATION,
    REQ_SUB_CHANNEL_1, REQ_SUB_CHANNEL_2  # <-- FIX: Naye Request FSub channels import kiye
)
...
async def start(client:Client, message): 
    ...
    # --- FIX: YAHAN PURANA 'AUTH_CHANNEL' BLOCK HATA KAR NAYA 'JOIN REQUEST FSUB' BLOCK DALA GAYA HAI ---
    
    # --- NAYA JOIN REQUEST FSUB BLOCK ---
    # Yeh check karega ki user ne dono channels par join request bheji hai ya nahi.
    
    user_id = message.from_user.id
    
    # Check karein ki user ne Channel 1 par request bheji hai
    has_req_1 = True  # Agar channel set nahi hai toh True maanein
    if REQ_SUB_CHANNEL_1:
        has_req_1 = await db.find_join_req(user_id, REQ_SUB_CHANNEL_1)

    # Check karein ki user ne Channel 2 par request bheji hai
    has_req_2 = True  # Agar channel set nahi hai toh True maanein
    if REQ_SUB_CHANNEL_2:
        has_req_2 = await db.find_join_req(user_id, REQ_SUB_CHANNEL_2)

    # Agar user ne dono mein se kisi ek par bhi request nahi bheji hai, toh usey rokein
    if not has_req_1 or not has_req_2:
        try:
            # Dono channels ke liye invite links banayein
            link_1 = None
            link_2 = None
            if not has_req_1 and REQ_SUB_CHANNEL_1:
                link_1 = await client.create_chat_invite_link(int(REQ_SUB_CHANNEL_1), creates_join_request=True)
            if not has_req_2 and REQ_SUB_CHANNEL_2:
        ...
        # Sirf unnhi channels ke button dikhayein jinke liye request pending hai
        btn = []
        if link_1:
            btn.append([InlineKeyboardButton("➡️ Channel 1 Join Karein ⬅️", url=link_1.invite_link)])
        if link_2:
            btn.append([InlineKeyboardButton("➡️ Channel 2 Join Karein ⬅️", url=link_2.invite_link)])

        # "Try Again" button add karein taaki user file/verification dobara pa sake
        if message.command[1] != "subscribe":
            try:
                # File request ko Ttry Again mein save karein
                kk, grp_id, file_id = message.command[1].split('_', 2)
                pre = 'checksubp' if kk == 'filep' else 'checksub'
                btn.append(
                    [InlineKeyboardButton("♻️ Request Bhejne Ke Baad Yahan Click Karein ♻️", callback_data=f"{pre}#{grp_id}#{file_id}")]
                )
            except (IndexError, ValueError):
                # Normal start command ko Try Again mein save karein
                btn.append(
                    [InlineKeyboardButton("♻️ Request Bhejne Ke Baad Yahan Click Karein ♻️", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")]
                )
        ...
        await client.send_message(
            ...
            text=(
                "**❌ Access Denied ❌**\n\n"
                "Aapko file ya verification link paane ke liye pehle hamare **dono** private channels par join request bhejni hogi.\n\n"
                "1. Neeche diye gaye button(s) par click karein.\n"
                "2. Channel par **'Request to Join'** button dabayein.\n"
                "3. Jab dono channels par request bhej dein, tab **'Request Bhejne Ke Baad Yahan Click Karein'** button dabayein."
            ),
            reply_markup=InlineKeyboardMarkup(btn),
    ...
    # --- NAYA JOIN REQUEST FSUB BLOCK KHATAM ---
...
async def delete(bot, message):
    ...
    if result.deleted_count:
        await msg.edit('<b>ꜰɪʟᴇ ɪs sᴜᴄᴄᴇssꜰᴜʟʟY ᴅᴇʟᴇᴛᴇᴅ ꜰʀᴏᴍ ᴅᴀᴛᴀʙᴀsᴇ 💥</b>')
    else:
...
async def send_msg(bot, message):
    ...
    if message.reply_to_message:
        ...
        try:
            ...
            await message.reply_text(f"<b>✅️ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟY ᴍᴇꜱꜱᴀɢᴇ ꜱᴇɴᴛ ɪɴ `{success_count}` ɪᴅ\n<code>{out}</code></b>")
        except Exception as e:
...
async def set_shortner(c, m):
    ...
    try:
        ...
        await m.reply_text(f"<b><u>✅ sᴜᴄᴄᴇssꜰᴜʟʟY ʏᴏᴜʀ sʜᴏʀᴛɴᴇʀ ɪs ᴀᴅᴅᴇᴅ</u>\n\nᴅᴇᴍᴏ - {SHORT_LINK}\n\nsɪᴛᴇ - `{URL}`\n\nᴀᴘɪ - `{API}`</b>", quote=True)
        user_id = m.from_user.id
...
async def set_log(client, message):
    ...
    await message.reply_text(f"<b>✅ sᴜᴄᴄᴇssꜰᴜʟʟY sᴇᴛ ʏᴏᴜʀ ʟᴏɢ ᴄʜᴀɴɴᴇʟ ꜰᴏʀ {title}\n\nɪᴅ - `{log}`</b>", disable_web_page_preview=True)
...
# --- FIX: 'welcome_handler' MEIN 'AttributeError' BUG KO THEEK KIYA GAYA ---
...
async def welcome_handler(client: Client, member: ChatMemberUpdated):
    try:
        ...
        if (
            ...
            # 2. Re-join check (REMOVED) - Logic aapke code ke hisab se
            ...
            # --- YEH HAI ASLI FIX ---
            # Hum referrer ka 'mention' text pehle hi nikal lenge.
            # Agar client.get_users fail hota hai (kyunki bot ne user ko nahi dekha hai), 
            # toh hum ek 'fallback mention' text banayenge.
            
            referrer_mention = f"User ({referrer_id})" # Default fallback
            try:
                referrer_user = await client.get_users(referrer_id)
                referrer_mention = referrer_user.mention # Agar user mila toh mention update karo
            except Exception as e:
                # Agar user fetch karne mein error aaye toh log kar sakte hain
                logger.warning(f"Could not fetch referrer user {referrer_id}: {e}")
                # referrer_mention default text hi rahega
            ...
            if new_count >= REFERRAL_TARGET:
                ...
                try:
                    await client.send_message(
                        chat_id=referrer_id,
                        # FIX: Yahan 'referrer_mention' variable ka istemal karein
