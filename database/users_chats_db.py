...
class Database:
    ...
    def __init__(self):
        ...
        # FIX: Advanced FSub - 'req' ko behtar naam diya aur naya collection use kiya
    ...
    # FIX: Advanced FSub - 'find_join_req' ko update kiya taaki channel_id bhi le
    ...
    # FIX: Advanced FSub - 'add_join_req' ko update kiya taaki channel_id bhi le
    async def add_join_req(self, user_id, channel_id):
        # upsert=True duplicate entries ko rokega
    ...
    # FIX: Advanced FSub - 'del_join_req' ko naye collection par point kiya
    async def del_join_req(self):
        # Admin command (/delreq) ke liye
        await self.pending_requests.drop()

    # FIX: Advanced FSub - Naya function add kiya taaki approved/declined users ko remove kar sakein
    async def remove_join_req(self, user_id, channel_id):
        # User ko specific channel ki pending list se remove karein
    ...
    # FIX: Referral System - 'get_user_by_referral_link' ko fix kiya
    async def get_user_by_referral_link(self, link):
        # Naye 'referral_links' collection se link document search karein
        link_doc = await self.ref_links.find_one({'_id': link})
        
        if link_doc:
            # Link mila. Ab link se 'referrer_id' ka istemal karke
            # main user document (users collection se) return karein
            referrer_id = link_doc.get('referrer_id')
            return await self.col.find_one({'id': int(referrer_id)})
        
        # Agar link nahi mila, toh None return karein
        return None
