...
class Database:
    ...
    def __init__(self):
        ...
        self.users = self.col 
        self.req = mydb.requests # Purani requests ke liye (optional)
        self.ref_links = mydb.referral_links
        self.referrals = mydb.referrals
        self.join_reqs = mydb.join_requests  # Naya join requests collection
    ...
    # --- YEH HAIN FIX ---
    # Purane join_req functions ko naye functions se badal diya gaya hai
    
    async def find_join_req(self, user_id, chat_id):
        # Check karein ki user ne specific channel ke liye request ki hai
        return bool(await self.join_reqs.find_one({
            'user_id': user_id,
            'chat_id': chat_id
        }))
    ...
    async def add_join_req(self, user_id, chat_id):
        ...
        await self.join_reqs.update_one(
