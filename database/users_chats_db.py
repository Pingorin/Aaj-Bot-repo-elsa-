...
class Database:
    ...
    def new_user(self, id, name):
        return dict(
            ...
            referral_count=0,
            fsub_passed=False  # <-- FIX 1: Permanent FSub status
        )
    ...
    # --- FIX 2: Updated FSub functions ---
    async def find_join_req(self, user_id, chat_id):
        # Check karein ki user ne specific channel ke liye request bheji hai
        return bool(await self.req.find_one({'user_id': user_id, 'chat_id': chat_id}))
        
    async def add_join_req(self, user_id, chat_id):
        # User ki request ko user_id aur chat_id ke saath save karein
        # Duplicates se bachne ke liye update_one aur upsert=True ka istemal karein
    ...
    async def del_join_req(self):
        # FIX 3: .drop() ke bajaye .delete_many({}) ka istemal kiya gaya hai
    ...
    # --- FIX 4: Add new 'fsub_passed' functions ---
    async def get_fsub_status(self, user_id):
        user = await self.col.find_one({'id': int(user_id)})
        if not user:
            return False
        return user.get('fsub_passed', False)

    async def update_fsub_status(self, user_id, status=True):
        await self.col.update_one(
            {'id': int(user_id)},
            {'$set': {'fsub_passed': status}}
        )
    # --- End of 'fsub_passed' functions ---
