...
class Database:
    ...
    def __init__(self):
        ...
        # --- NAYA FSUB COLLECTION ---
        self.pending_reqs = mydb.pending_requests
    ...
    async def get_notcopy_user(self, user_id):
        ...
        if not user:
            ...
            # Bug fix: 'user' was not awaited, should be insert_one
            await self.misc.insert_one(res)
            user = res # Use the dict we just created
        return user
    ...
    async def add_pending_request(self, user_id, channel_id):
        """User ki join request ko 'pending' olarak database mein store karein."""
        await self.pending_reqs.update_one(
            {'user_id': user_id, 'channel_id': channel_id},
            {'$set': {'status': 'pending', 'time': datetime.datetime.now()}},
            upsert=True
        )

    async def remove_pending_request(self, user_id, channel_id):
        """User ko 'pending' database se hatayein (jab woh approve/decline/cancel ho)."""
        await self.pending_reqs.delete_one(
            {'user_id': user_id, 'channel_id': channel_id}
        )

    async def is_user_pending(self, user_id, channel_id):
        """Check karein ki user abhi bhi 'pending' hai ya nahi."""
        return bool(await self.pending_reqs.find_one(
            {'user_id': user_id, 'channel_id': channel_id}
        ))
