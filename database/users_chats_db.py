...
class Database:
    ...
    def __init__(self):
        ...
        self.users = self.col 
        self.req = mydb.requests # Purana Fsub collection (ise chhod dein)
    ...
    # --- OLD FSUB FUNCTIONS (Ab poori tarah hata diye gaye hain) ---
    ...
    async def add_join_request(self, user_id, chat_id):
        ...
        await self.join_requests.update_one(
            ...
            # FIX: Added timezone-aware datetime
    ...
    # --- FIX: Function rename kiya gaya taaki /delreq command chale ---
    ...
    async def has_premium_access(self, user_id):
        ...
        if user_data:
            ...
            if expiry_time is None:
                return False
            # FIX: Use datetime.datetime.now(pytz.utc) for timezone-aware comparison
            elif isinstance(expiry_time, datetime.datetime) and datetime.datetime.now(pytz.utc) <= expiry_time:
                return True
            else:
    ...
    async def get_expired(self, current_time):
        expired_users = []
        # FIX: Use current_time (which should be timezone-aware)
    ...
    async def log_referral(self, new_user_id, referrer_id, chat_id):
        ...
        await self.referrals.insert_one({
            ...
            'chat_id': chat_id,
            'timestamp': datetime.datetime.now(pytz.utc) # Added timestamp
        })
