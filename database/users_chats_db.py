...
class Database:
    ...
    async def set_referred_by(self, new_user_id, referrer_id):
        # This function is called when a new user joins via link
    ...
    async def has_been_referred(self, user_id):
        user = await self.col.find_one({'id': int(user_id)})
        if user:
            return user.get('referred_by') is not None
        return False
